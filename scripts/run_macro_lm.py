...
Mirrors scripts/run_covid_lm.py. Reads parameters from env vars set by
condor/run_one.sh and executes one (training_style, kl_beta) configuration
of the AT macro_economics sim driven by a HuggingFace causal LM that
recommends per-agent consumption propensity.

What the LM is doing:
  prompt: agent profile (age, gender, ethnicity)
  output: a number in [0, 1] = recommended consumption propensity
  injected: as platform_signal (a logit), AT runner applies sigmoid in
            PerfsimEarningDecision → consumption_propensity
  agent effect: gates assets-to-consumption flow in UpdateAssetsGoods
  macro effect: aggregates into demand → prices, inflation, unemployment

TODOs for the user to refine in real experiments (search "TODO:" in this
file):
  - target_extractor's economic-risk-aware target (currently a coarse
    income-bucket-based recommendation; should depend on current macro
    state for the performative loop to be meaningful)
  - which features go into the prompt (age + gender + ethnicity now;
    area + region could be added once bucket counts are tractable)
  - metric set (currently mean_assets, mean_consumption; you'll want
    inflation, unemployment_rate, price_of_goods captured per round)
"""
...
def main() -> int:
    ...
    # ---- Target extractor -------------------------------------------------
    # TODO: refine this. For now, use a simple risk-based recommendation
    # bucketed on (age, gender) with a small shift for current macro
    # state. Closer to "income-bucket recommendation with inflation-aware
    # adjustment". This is a starter target; you'll want to make it
    # genuinely performative (depend on aggregate state changes).
