"""Calibrate AT macro_economics ABM to real Queens unemployment data.

Fits the unemployment_adaptation_coefficient (UAC) so the ABM's
unemployment trajectory matches real Queens County monthly rates.

The bundled data has Queens unemployment from 2019-2023. The macro ABM
runs in monthly timesteps. We fit UAC (the regression coefficients in
the unemployment equation) via gradient descent through the ABM.

Usage:
  python scripts/calibrate_macro.py
  python scripts/calibrate_macro.py --n-iters 50 --lr 0.1
"""
