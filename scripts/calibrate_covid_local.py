"""Calibrate AT covid to real Astoria winter surge data.

Jointly fits R2 (transmission rate) and initial infection fraction so the
ABM's total infections match real Astoria case counts.

Strategy: before each forward pass, use a differentiable soft-seeding that
sets disease_stage to a mix of S and I controlled by a learnable parameter.
This lets the optimizer find both the right transmission rate AND the right
initial conditions.

Usage: python scripts/calibrate_covid_local.py
"""
