"""Block 13b: two prior-anchored MLP platforms, hard per-agent assignment.

Same setup as block 13 (distinct PA=0.3/PB=0.7 vs shared 0.7/0.7) but the
feedback rule matches the FJ LLM runs: among in-gate platforms an agent is
assigned to the single NEAREST prediction and blends toward that one only.
Each platform trains only on its assigned agents.

Run: MPLCONFIGDIR=/tmp/mpl python experiments/competition/13b_two_priors_hard.py
"""
...
def run(eps, gamma, priors, seed):
    ...
    for t in range(ROUNDS):
        ...
        dist = np.stack([np.abs(m - x) for m in ms])
        eligible = dist < eps
        # nearest eligible platform, -1 if none
        nearest = dist.argmin(0)
        assigned = np.where(eligible[nearest, np.arange(N)], nearest, -1)
        for k, (net, opt, prior) in enumerate(zip(nets, opts, priors)):
            aud = torch.tensor(assigned == k)
            if aud.sum() < MIN_AUD:
                continue
            for _ in range(TRAIN_STEPS):
                ...
                loss.backward()
                opt.step()
        blend_to = np.where(assigned == 0, ms[0], ms[1])
        x_new = np.where(assigned >= 0, (1 - W) * x + W * blend_to, x)
        for i in range(N):
...
def main():
    res = {}
    print(f"AB n={N} complete graph, two platforms (hard assignment), w={W}, "
        ...
        for eps in EPS_GRID:
            for cond, priors in CONDS.items():
                ...
                print(f"{gamma:>6.2f} {eps:>5.2f} {cond:>9} | "
                      ...
                      flush=True)
    json.dump(res, open("experiments/competition/figs/two_priors_hard.json", "w"))
    ...
    fig.suptitle("Two platforms, hard assignment: distinct (0.3/0.7) vs shared (0.7)")
    fig.savefig("experiments/competition/figs/two_priors_hard.png", dpi=130)
    print("saved experiments/competition/figs/two_priors_hard.png")
