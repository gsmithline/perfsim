...
from perfsim.models.openrouter_client import (            # noqa: E402
    Budget, DecodingPolicy, OpenRouterClient, ProviderPin, load_api_key,
    validate_key,
)
...
def main() -> int:
    ...
    try:
        meta = validate_key()
        print(f"[canary] key OK: label={meta.get('label')!r} "
    ...
    pin = ProviderPin(order=(args.provider,), allow_fallbacks=False,
                      ...
                      zdr=True)
    policy = DecodingPolicy(temperature=0.0, top_p=1.0,
    ...
    client = OpenRouterClient(model=args.model, provider=pin, policy=policy,
    ...
    print(f"[canary] sending {n} request(s) to {args.model} via "
