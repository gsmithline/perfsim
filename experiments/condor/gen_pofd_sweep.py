...
ROW_UD = ("{tag}, sft, 0, 0, 1, replace, 1.0, fixed, ab, 0.2, 0.0, 1, "
          "loop, 0.0, 1, forward, 100, {accum}, {iclk}, {snap}, "
          "{uselora}, {fresh}, {ansk}, {gg}, {nrounds}, {basemodel}, "
          "{chatthink}, {mem}, {disk}, {pplbatch}")


def ud_tag(u, rounds=UD_ROUNDS, smoke=False):
    pre = "pofdudsmk" if smoke else "pofdud"
    return (f"{pre}_{UD_MODEL}_sft_u{u}_sw100_eaopen_w1_k1_esopen_"
            f"{S3_OP_TOKEN}_s0_r{rounds}")


def ud_row(u, accum, rounds=UD_ROUNDS, smoke=False):
    a = REACH_ARM_COLS["b0"]
    m = FAM_MODELS[UD_MODEL]
    return ROW_UD.format(
        tag=ud_tag(u, rounds, smoke), accum=accum,
        iclk=a["iclk"], snap=a["snap"], uselora=a["uselora"],
        fresh=a["fresh"], ansk=a["ansk"], gg=a["gg"], nrounds=rounds,
        basemodel=m["base_model"], chatthink=m["chatthink"],
        mem=m["mem"], disk=m["disk"], pplbatch=m["pplbatch"])
...
def ud_sub(smoke=False):
    rows = ud_smoke_rows() if smoke else ud_rows()
    key = UD_SMOKE_KEY if smoke else UD_KEY
    return UD_SUB_TEMPLATE.format(
        ...
        kind=("2-ROUND u1 SMOKE" if smoke
              else "RECURSIVE UPDATE-DOSE, 10 ROUNDS"),
        gateflag=(" --smoke" if smoke else ""))
...
# AB_SWEEPS = 100, fresh LoRA r512, plain SFT, seed 0, {rounds} rounds.
...
# checker against trajectory.pt's sft_dose records; u181 (accum 1) is
# NOT queued -- the archived pofdps_ SFT cell serves it.
