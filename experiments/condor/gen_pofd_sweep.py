...
request_disk      = $(disk)
request_gpus      = 1
requirements      = (TARGET.CUDAGlobalMemoryMb >= 80000) && (TARGET.CUDADeviceName == "{gpu}") && (TARGET.Machine =!= MY.LastRemoteHost){bad}
...
# agent's ORIGINAL frozen-Qwen prediction, held fixed for the whole run:
#     y_i = x_i(t)  if i in S_t   else   b_i
# with |S_t| = round(q*n) drawn from a stateless permutation of
# (seed, round), so smaller-q arms are NESTED inside larger ones, and
# q = 1 is exactly ordinary SFT.
#
# This is the EXISTING REF_REPLAY_* path -- no new mechanism. b comes
# from the canonical frozen Qwen3 run's pred_raw[0]. Verified compatible:
# REF_REPLAY refuses REPLAY_FRAC, PRISTINE_FRAC, SFT_SAMPLE_N, a CUTTING
# TRAIN_CAP, SFT_EXCLUDE_CLAMPED, DATA_REGIME!=replace and DPO, and this
# key sets none of them (train_cap == n_labeled == 723 is allowed; the
# guard is 0 < cap < n).
#
...
def mix_reused_tag():
    """The ordinary-SFT cell that serves q = 1, as a FUNCTION so it can
    name a tag another key builds without depending on definition order."""
    return ("pofdps_qwen3_8b_sft_sw100_eaopen_w1_k1_esopen"
            f"_{S3_OP_TOKEN}_s0_r60")
...
def mix_tag(q, rounds=MIX_ROUNDS, smoke=False):
    """pofdmix_qwen3_8b_q0p1_sw100_eaopen_w1_k1_esopen_anch2_s0_r5."""
    pre = "pofdmixsmk" if smoke else "pofdmix"
    return (f"{pre}_{MIX_MODEL}_q{_num(q)}_sw{MIX_SWEEPS}_eaopen"
            f"_w{_num(MIX_W)}_k{_num(MIX_K)}_esopen_{S3_OP_TOKEN}"
            f"_s{MIX_SEED}_r{rounds}")


def mix_row(q, rounds=MIX_ROUNDS, smoke=False):
    a = REACH_ARM_COLS["b0"]
    m = FAM_MODELS[MIX_MODEL]
...
def mix_rows():
    return [mix_row(q) for q in MIX_QS]


def mix_smoke_rows():
    """q = 0.1 -- the most-replaced arm, i.e. the one where the mixture
    machinery does the most work and a broken b would show fastest."""
    return [mix_row(0.1, rounds=MIX_SMOKE_ROUNDS, smoke=True)]


def mix_sub(smoke=False):
    rows = mix_smoke_rows() if smoke else mix_rows()
    key = MIX_SMOKE_KEY if smoke else MIX_KEY
    return MIX_SUB_TEMPLATE.format(
        key=key, n_jobs=len(rows), gpu=MIX_H100, bad=BAD_NODE_REQ,
        rounds=(MIX_SMOKE_ROUNDS if smoke else MIX_ROUNDS),
        refrun=MIX_REF_RUN, selseed=MIX_SEL_SEED,
        kind=("3-ROUND q=0.1 SMOKE" if smoke
              else "FIXED/LIVE LABEL MIXTURE, 5-ROUND DIRECTIONAL TEST"))
...
LAM_KEY = "section3_lambda_fill"
LAM_SMOKE_KEY = "section3_lambda_fill_smoke"
LAM_MODEL = "qwen3_8b"
LAM_SWEEPS = 100
LAM_SEED = 0
LAM_SMOKE_ROUNDS = 3
LAM_H100 = S3_H100
LAM_EPS_SOCIAL = S3_EPS_SOCIAL
...
def lam_tag(lam, w, k, rounds, smoke=False):
    pre = "pofdlamsmk" if smoke else "pofdlam"
    return (f"{pre}_{LAM_MODEL}_fwdlam{_num(lam)}_sw{LAM_SWEEPS}_eaopen"
            f"_w{_num(w)}_k{_num(k)}_esopen_{S3_OP_TOKEN}_s{LAM_SEED}"
            f"_r{rounds}")


def lam_row(lam, w, k, rounds, smoke=False):
    a = REACH_ARM_COLS["b1"]
    m = FAM_MODELS[LAM_MODEL]
    return ROW_PS.format(
        tag=lam_tag(lam, w, k, rounds, smoke), style="sft_kl",
        beta=f"{lam:g}", seed=LAM_SEED, es=f"{LAM_EPS_SOCIAL:g}",
        wplat=f"{w:g}", lam=f"{k:g}", kldir="forward", sweeps=LAM_SWEEPS,
        iclk=a["iclk"], snap=a["snap"], uselora=a["uselora"],
        fresh=a["fresh"], ansk=a["ansk"], gg=a["gg"], nrounds=rounds,
        basemodel=m["base_model"], chatthink=m["chatthink"],
        mem=m["mem"], disk=m["disk"], pplbatch=m["pplbatch"])
...
def lam_rows():
    return [lam_row(l, w, k, r)
            for (w, k, r) in LAM_FAMILIES for l in LAM_NEW]


def lam_smoke_rows():
    """Strongest new dose in the family with the most state carryover."""
    return [lam_row(4.0, 0.5, 0.2, LAM_SMOKE_ROUNDS, smoke=True)]


def lam_sub(smoke=False):
    rows = lam_smoke_rows() if smoke else lam_rows()
...
MIX_SUB_TEMPLATE = """\
# HTCondor: SECTION-3 FIXED/LIVE LABEL MIXTURE -- {kind}, {n_jobs} jobs.
# GENERATED by gen_pofd_sweep.py from the MIX block. Never edit by hand.
# Qwen3-8B, movielens/Action 723, W = 1, k = 1, both gates all_open,
# AB_SWEEPS = {rounds}-round horizon with S = 100 complete Deffuant
# sweeps per round, seed 0, fresh LoRA r512, ordinary SFT.
# THE MIXTURE. Every round keeps all 723 rows; q of them carry the
# agent's CURRENT post-peer opinion and 1-q carry that agent's ORIGINAL
# frozen-Qwen prediction. q rides the queue as $(refq); the selection
# stream and the reference run are pinned here, so every row of this key
# shares them and the subsets are NESTED across q.
# q = 1 is ordinary SFT and is NOT queued -- the archived
# pofdps_qwen3_8b_sft_sw100_... cell serves it.
...
#
# SAVE_RAW_GEN=1 IS MANDATORY (2026-08-25 review). An earlier draft left
# it off to match the archived Section-4 cells, arguing the checker could
# gate parse failures on non-finite pred_raw. That was WRONG: the parser
# falls back to a FINITE 0.5 on failure (run_pokec_gated_lm.py, "the
# parser fell back to its 0.5 default"), so a wave with widespread parse
# failures would look perfectly finite. parse_fail_frac exists ONLY in
# raw_gen_log.json.gz, so every Section-4 job (smoke, production,
# extension) writes it and the checker REQUIRES the file plus zero
# failures. save_raw_gen is a config field, but nothing archived is
# reused here (reuse audit = 0), so nothing is made incomparable.
#
...
# section3_model_equilibria[_smoke] (2026-08-25).  OPEN-GATE,
# CROSS-MODEL CONSENSUS COMPARISON requested for the Section 3 flow.
#
# QUESTION.  Jiduan's perfect-prediction baseline reaches consensus while
# preserving the innate population mean.  If a fresh adapter is instead
# reference-regularized toward an entering pretrained model, does consensus
# form at a MODEL-SPECIFIC mean?
#
# EXACT SURFACE.  The six paper checkpoints share beta=W_PLAT=1,
# gamma=INNATE_LAMBDA=1 (inert when beta=1), Deffuant alpha=0.5 (the
# production default), both gates genuinely all_open, S=100 complete peer
# sweeps per round, forward KL lambda=2, fresh r512 LoRA each round, 30 outer
# rounds, MovieLens/Action 723, seeds {0,42,43}.  Only BASE_MODEL and the
# replication seed change.  The figure
# contains the six lambda=2 outcomes and one perfect-prediction reference at
# the common innate mean; it deliberately contains NO lambda=0 series because
# the full lambda ladder in Figure 3 already establishes the no-shift regime.
#
# A Qwen3-8B seed-0 result exists at this surface, but this compact wave
# deliberately reruns it so every plotted value shares one code provenance.
# Therefore the wave is 18 new GPU jobs (six models x three seeds), plus one
# 3-round OLMo-3 smoke that exercises a non-Qwen checkpoint on the new key.
S3M_KEY = "section3_model_equilibria"
S3M_SMOKE_KEY = "section3_model_equilibria_smoke"
S3M_MODELS = ("qwen7b", "qwen3_8b", "olmo7b", "olmo3_7b",
              "mistral7b", "ministral8b")
S3M_REUSED = {}
S3M_BETA = 1.0
S3M_GAMMA = 1.0
S3M_LAMBDA = 2.0
S3M_ALPHA = 0.5                 # gp.ab_sweep production default
S3M_SWEEPS = 100
S3M_ROUNDS = 30
S3M_SMOKE_ROUNDS = 3
S3M_SEEDS = (0, 42, 43)
S3M_SMOKE_SEED = 991
S3M_SMOKE_MODEL = "olmo3_7b"
S3M_H100 = S3_H100
...
def s3m_rows():
    """One fresh, common-provenance row per model and replication seed."""
    return [s3m_row(model, seed) for model in S3M_MODELS
            for seed in S3M_SEEDS if (model, seed) not in S3M_REUSED]


def s3m_smoke_rows():
    return [s3m_row(S3M_SMOKE_MODEL, S3M_SMOKE_SEED,
...
S3M_SUB_TEMPLATE = """\
# HTCondor: SECTION-3 MODEL-SPECIFIC EQUILIBRIA -- {kind}, {n_jobs} jobs.
# GENERATED by gen_pofd_sweep.py from the S3M block. Never edit by hand.
# Six checkpoints x seeds {{0,42,43}} at one matched surface: 18 production
# jobs. Existing older cells are intentionally not reused so every plotted
# value shares one code provenance.
# beta=W_PLAT=1, gamma=INNATE_LAMBDA=1 (inert at beta=1), forward-KL
# lambda=2, Deffuant alpha=0.5 (production default), S=100 complete peer
# sweeps, both gates all_open, fresh r512 LoRA each round, {rounds} rounds,
# MovieLens/Action 723. Only BASE_MODEL and replication seed change.
# This wave has NO lambda=0 arm. Perfect prediction is the common innate-mean
# reference and requires no GPU run.
# Gate: python experiments/scripts/cluster_pipelines/check_section3_model_equilibria.py{gateflag}
# Submit: bash experiments/condor/submit_pofd_sweep.sh <BID> {key}
universe          = vanilla
executable        = /home/gsmithline/perfsim/experiments/condor/run_one_pokec_gated_idempotent.sh
arguments         = $(tag) $(style) $(beta) $(seed) $(deploy_every) $(regime) $(pscale) $(anchor) $(pop) $(eps) $(gamma) $(wplat) $(mode) $(canary)
...
# W = 1, k = 1 inert, both gates open, S = 100, fresh LoRA, seed 0), does
# the optimizer-STEP FREQUENCY of plain SFT change the closed-loop
# trajectory when the DATA EXPOSURE is held fixed? Every arm consumes all
# 723 current live labels exactly once per round in the same sampler
# order (SAVE_SFT_ORDER=1 persists it; the trainer seed is recorded);
# only SFT_GRAD_ACCUM varies, splitting the single pass into averaged
# update blocks. NOT the pofdsftdose_ step-CAP family: that dial cuts
# rows seen (4U of 723); this one never does.
#
# THE ARMS. 723 rows at microbatch 4 = 181 minibatches, and 181 is
# PRIME, so an exact 20-block partition does not exist; the realizable
# ladder is global_step = ceil(181/accum):
#   u1    accum 181  -> 1 averaged full-batch step
#   u5    accum  37  -> 5 blocks
#   u19   accum  10  -> 19 blocks (the closest realizable to "20")
#   u181  accum   1  -> ordinary minibatch SFT == the ARCHIVED
#         pofdps_qwen3_8b_sft_sw100_..._r60 cell, REUSED (its first 10
#         rounds; SFT_GRAD_ACCUM=1 is the byte-identical legacy path).
# Tags carry the REALIZED step count, so no tag can promise 20 and run 19.
# CONTROL: the matched perfect-prediction trajectory is the archived CPU
# artifact pp_k1_w1_eaopen_esopen_sw100_s0_r30.pt -- reused, zero jobs.
# => 3 NEW GPU jobs (10 rounds) + 1 two-round u1 smoke.
UD_KEY = "sft_update_dose_loop"
UD_SMOKE_KEY = "sft_update_dose_loop_smoke"
UD_MODEL = "qwen3_8b"
UD_ROUNDS = 10
UD_SMOKE_ROUNDS = 2
UD_H100 = S3_H100
UD_ARMS = [(1, 181), (5, 37), (19, 10)]      # (realized steps, accum)
UD_REUSED_U181 = ("pofdps_qwen3_8b_sft_sw100_eaopen_w1_k1"
                  "_esopen_anch2_s0_r60")
UD_PP_CONTROL = "pp_k1_w1_eaopen_esopen_sw100_s0_r30.pt"

# SEED REPLICATION of the update-dose test (2026-08-28, user). The
# reviewer wants the U ladder replicated beyond one seed, so seeds 42 and
# 43 are added at U in {1, 5, 181} -- ten rounds, EVERY other setting
# byte-identical to the seed-0 cells (same template, same accum map, same
# PARSE_MODE default, same S=100, same fresh LoRA). U=19 is deliberately
# NOT replicated: the reviewer named {1, 5, 181} and widening the ladder
# would turn a replication into a new grid.
#   3 doses x 2 seeds = 6 jobs, none of which can overwrite anything:
#   u1/u5 exist only at s0, and u181 has never been run under a pofdud_
#   tag at any seed.
# THE u181 ARM. accum = 1 is ordinary minibatch SFT: run_pokec_gated_lm
# only sets gradient_accumulation_steps when SFT_GRAD_ACCUM > 1, so this
# arm takes the byte-identical legacy trainer path -- the most-exercised
# path in the repository, and the same one the archived pofdps_ cell that
# serves u181 at seed 0 took. No new runtime code path is introduced by
# this wave, so no GPU smoke is warranted; what IS new is checker code,
# which is exercised against the three completed seed-0 cells instead.
UD_SEEDS_KEY = "sft_update_dose_loop_seeds"
UD_EXT_SEEDS = (42, 43)
UD_EXT_ARMS = [(1, 181), (5, 37), (181, 1)]   # (realized steps, accum)
UD_N_EXT = len(UD_EXT_ARMS) * len(UD_EXT_SEEDS)

ROW_UD = ("{tag}, sft, 0, {seed}, 1, replace, 1.0, fixed, ab, 0.2, 0.0, 1, "
          "loop, 0.0, 1, forward, 100, {accum}, {iclk}, {snap}, "
          "{uselora}, {fresh}, {ansk}, {gg}, {nrounds}, {basemodel}, "
          "{chatthink}, {mem}, {disk}, {pplbatch}")


def ud_tag(u, rounds=UD_ROUNDS, smoke=False, seed=0):
    pre = "pofdudsmk" if smoke else "pofdud"
    return (f"{pre}_{UD_MODEL}_sft_u{u}_sw100_eaopen_w1_k1_esopen_"
            f"{S3_OP_TOKEN}_s{seed}_r{rounds}")


def ud_row(u, accum, rounds=UD_ROUNDS, smoke=False, seed=0):
    a = REACH_ARM_COLS["b0"]
    m = FAM_MODELS[UD_MODEL]
    return ROW_UD.format(
        tag=ud_tag(u, rounds, smoke, seed), seed=seed, accum=accum,
        iclk=a["iclk"], snap=a["snap"], uselora=a["uselora"],
        fresh=a["fresh"], ansk=a["ansk"], gg=a["gg"], nrounds=rounds,
        basemodel=m["base_model"], chatthink=m["chatthink"],
        mem=m["mem"], disk=m["disk"], pplbatch=m["pplbatch"])
...
def ud_rows():
    return [ud_row(u, acc) for u, acc in UD_ARMS]


def ud_ext_rows():
    """Seeds 42 and 43 at U in {1, 5, 181}; seed 0 already exists."""
    return [ud_row(u, acc, seed=s)
            for s in UD_EXT_SEEDS for u, acc in UD_EXT_ARMS]


def ud_smoke_rows():
    """u1 (accum 181): the maximally-averaged path, 2 rounds."""
    return [ud_row(1, 181, rounds=UD_SMOKE_ROUNDS, smoke=True)]


def ud_sub(smoke=False, ext=False):
    assert not (smoke and ext)
    rows = (ud_ext_rows() if ext else
            ud_smoke_rows() if smoke else ud_rows())
    key = UD_SEEDS_KEY if ext else UD_SMOKE_KEY if smoke else UD_KEY
    return UD_SUB_TEMPLATE.format(
        key=key, n_jobs=len(rows), gpu=UD_H100, bad=BAD_NODE_REQ,
        rounds=(UD_SMOKE_ROUNDS if smoke else UD_ROUNDS),
        kind=("2-ROUND u1 SMOKE" if smoke
              else "UPDATE-DOSE SEED REPLICATION, 10 ROUNDS"
              if ext else "RECURSIVE UPDATE-DOSE, 10 ROUNDS"),
        seedtxt=("seeds " + "/".join(str(s) for s in UD_EXT_SEEDS)
                 if ext else "seed 0"),
        u181note=("u181 (accum 1) IS queued here: no archived pofdps_ "
                  "cell exists at seeds 42/43, and accum 1 takes the "
                  "byte-identical legacy trainer path."
                  if ext else
                  "u181 (accum 1) is NOT queued -- the archived pofdps_ "
                  "SFT cell serves it."),
        gateflag=(" --smoke" if smoke else
                  " --seeds " + ",".join(str(s) for s in UD_EXT_SEEDS)
                  if ext else ""))


UD_SUB_TEMPLATE = """\
# HTCondor: RECURSIVE UPDATE-DOSE TEST -- {kind}, {n_jobs} jobs.
# GENERATED by gen_pofd_sweep.py from the UD block. Never edit by hand.
# Section 3 no-memory/full-adoption surface: Qwen3-8B (thinking OFF),
# movielens/Action 723, W = 1 (k = 1 inert), both gates all_open,
# AB_SWEEPS = 100, fresh LoRA r512, plain SFT, {seedtxt}, {rounds} rounds.
# ONLY SFT_GRAD_ACCUM varies (queue col accum): every arm consumes all
# 723 live labels exactly once per round (SFT_EPOCHS=1) in the same
# recorded sampler order (SAVE_SFT_ORDER=1); LR, rank, batch and data
# exposure are FIXED. Realized steps = ceil(181/accum), asserted by the
# checker against trajectory.pt's sft_dose records.
# {u181note}
# Gate: python experiments/scripts/cluster_pipelines/check_update_dose.py{gateflag}
# Submit: bash experiments/condor/submit_pofd_sweep.sh <BID> {key}
universe          = vanilla
executable        = /home/gsmithline/perfsim/experiments/condor/run_one_pokec_gated_idempotent.sh
arguments         = $(tag) $(style) $(beta) $(seed) $(deploy_every) $(regime) $(pscale) $(anchor) $(pop) $(eps) $(gamma) $(wplat) $(mode) $(canary)
...
getenv            = False
environment       = "REPO=/home/gsmithline/perfsim CONDA_SH=/home/gsmithline/miniconda3/etc/profile.d/conda.sh ENV_NAME=opdyn WANDB_KEY_FILE=/home/gsmithline/.wandb_key WANDB_PROJECT=perfsim-gated-lm DATASET=movielens ML_TARGET=Action HF_HOME=/lustre/fast/fast/gsmithline/hf_cache HF_HUB_OFFLINE=1 AI_GATE_MODE=all_open PEER_GATE_MODE=all_open AI_GATE_REFERENCE=anchor EPS_AI=1 INNATE_LAMBDA=$(lam) AB_SWEEPS=$(sweeps) SFT_GRAD_ACCUM=$(accum) SAVE_SFT_ORDER=1 ICL_K=$(iclk) ICL_SNAPSHOT_ROUND=$(snap) ICL_DAYS=0 ICL_SELECT=random ICL_CTX_SOURCE=live USE_LORA=$(uselora) FRESH_EACH_ROUND=$(fresh) ANS_SAMPLE_K=$(ansk) ANS_SAMPLE_N=64 ANS_SAMPLE_T=1.0 LOG_GENDER_GAPS=$(gg) KL_DIRECTION=$(kldir) WITH_TWIN=1 SAVE_RAW_GEN=1 CHAT_THINKING=$(chatthink) BASE_MODEL=$(basemodel) TRAIN_CAP=723 N_ROUNDS=$(nrounds) EPOCH_SIZE=100 SFT_EPOCHS=1 SFT_BATCH_SIZE=4 GEN_BATCH_SIZE=32 LORA_R=512 SFT_LR=5e-5 N_LABELED=723 HIST_BINS=50 LOG_PERPLEXITY=1 N_PERPLEXITY=64 LOG_PPL_DIST=1 PPL_DIST_CAP=0 PPL_BATCH=$(pplbatch) SEED_BASE_DATA=1 WANDB_RUN_SUFFIX=_sft_update_dose_loop"
...
# eps_AI = 0 cells are TWIN-DERIVED (0 jobs) except for FOUR WITNESS
# RUNS -- both methods x both conditions, es=.3, seed 0 -- that the checker
...
#   192 cells = 144 GPU (ea in {.1,.3,1}) + 4 witness GPU (ea=0)
#             + 44 twin-derived (ea=0, 0 jobs)          -> 148 jobs
...
# (arm, cond, es, seed) run at ea = 0 as structural witnesses: BOTH
# methods x BOTH source conditions (a matched pair per method), so the
# op_raw == twin_raw identity is verified on the stubborn-clamp path as
# well as the plain one
S4G2_EA0_WITNESS = [("b0", "fixed", 0.3, 0), ("b0", "evolving", 0.3, 0),
...
# WHOLE-NODE PACKING (2026-08-26). The H100 pool vanished from the cluster and
# the only free >= 80 GB GPUs are g181-g184 (8x A100-SXM4-80GB each), whose
# START policy admits WHOLE-NODE jobs only (RequestCpus == TotalCpus &&
# RequestMemory == TotalMemory && RequestGpus == TotalGpus && RequestDisk ==
# TotalDisk; g141 is reserved for another user). A node job therefore runs
# up to 8 cells at once, one per GPU, through experiments/condor/node_pack.py:
# every cell's env + arguments are rebuilt from ITS OWN generated sub and
# configs row and executed by the same idempotent wrapper, so artifacts,
# configs and per-tag logs are exactly the single-GPU ones. The node totals
# differ by 2 MB of RAM between the pairs (an exact-equality START), hence
# one sub per pair. Nobody else uses these nodes (min price 35).
F4A_NODE_KEY = F4A_KEY + "_node"
...
F4A_NODE_PAIRS = (                       # sub suffix -> exact node totals
    ...
    ("b", {"hosts": ("g182", "g184"), "cpus": 128, "memory_mb": 2051934,
           "gpus": 8, "disk_kb": 22345412368}),
)


def f4a_node_groups():
    """Ordered {group_id: [entry, ...]} covering every production tag once
    (8 per group, grid order) plus one smoke group (2 smoke cells + both
    zero-shot priors). Entries name the cell's own sub + configs so
    node_pack.py reproduces the single-GPU launch exactly."""
    def ent(tag, key):
        return {"tag": tag,
                "configs": f"experiments/condor/configs_pofd_{key}.txt",
                "sub": f"experiments/condor/at_pofd_{key}.sub"}
    tags = [r.split(",")[0].strip() for r in f4a_rows()]
    groups = {}
    for i in range(0, len(tags), F4A_NODE_CELLS_PER_JOB):
        groups[f"prod{i // F4A_NODE_CELLS_PER_JOB}"] = [
            ent(t, F4A_KEY) for t in tags[i:i + F4A_NODE_CELLS_PER_JOB]]
    smoke = [ent(r.split(",")[0].strip(), F4A_SMOKE_KEY)
             for r in f4a_smoke_rows()]
    smoke += [ent(r.split(",")[0].strip(), F4A_ZSPRIOR_KEY)
              for r in f4a_zsprior_rows()]
    groups["smoke0"] = smoke
    return groups


def f4a_node_manifest_text():
    return json.dumps({"wave": F4A_KEY, "cells_per_job": F4A_NODE_CELLS_PER_JOB,
                       "hardware": F4A_GPU_NAME, "groups": f4a_node_groups()},
                      indent=1, sort_keys=True) + "\n"


def f4a_node_pair_of(group_id):
    """Production groups alternate between the two node pairs; smoke -> a."""
    if group_id == "smoke0":
        return "a"
    return "ab"[int(group_id[len("prod"):]) % 2]


def f4a_node_rows(pair, smoke=False):
    gids = [g for g in f4a_node_groups()
            if (g == "smoke0") == smoke and f4a_node_pair_of(g) == pair]
    return gids


def f4a_node_sub(pair, smoke=False):
    spec = dict(F4A_NODE_PAIRS)[pair]
    key = (F4A_NODE_SMOKE_KEY if smoke else F4A_NODE_KEY) + f"_{pair}"
    rows = f4a_node_rows(pair, smoke)
    hosts = " || ".join(f'(TARGET.Machine == "{h}.internal.cluster.is.localnet")'
                        for h in spec["hosts"])
    return F4A_NODE_SUB_TEMPLATE.format(
        key=key, n_jobs=len(rows), kind=("SMOKE" if smoke else "PRODUCTION"),
        pair=pair, hosts=", ".join(spec["hosts"]), hosts_req=hosts,
        cpus=spec["cpus"], memory_mb=spec["memory_mb"], gpus=spec["gpus"],
        disk_kb=spec["disk_kb"], gpu=F4A_GPU_NAME,
        manifest=os.path.basename(F4A_NODE_MANIFEST))


F4A_NODE_SUB_TEMPLATE = """\
# HTCondor: FIGURE-4 ANCHOR TRADE-OFF -- WHOLE-NODE {kind} (pair {pair}: {hosts}), {n_jobs} node job(s).
# GENERATED by gen_pofd_sweep.py from the F4A node-pack block. Never edit by hand.
# Each node job runs one manifest group (<= 8 cells, one per A100) through
# experiments/condor/run_node_pack.sh -> node_pack.py, which rebuilds every
# cell's env/args from its own single-GPU sub + configs row and runs the
# same idempotent wrapper: per-tag artifacts and logs are the ordinary ones.
# The request equals the node totals EXACTLY (the nodes' START policy is an
# equality on all four resources). Hold/release: a failed cell makes the
# node job exit non-zero; on release the wrapper skips completed cells.
universe          = vanilla
executable        = /home/gsmithline/perfsim/experiments/condor/run_node_pack.sh
arguments         = experiments/condor/{manifest} $(group)

request_cpus      = {cpus}
request_memory    = {memory_mb}
request_disk      = {disk_kb}
request_gpus      = {gpus}
requirements      = (TARGET.CUDADeviceName == "{gpu}") && (TARGET.TotalCpus == {cpus}) && (TARGET.TotalMemory == {memory_mb}) && (TARGET.TotalGpus == {gpus}) && (TARGET.TotalDisk == {disk_kb}) && ({hosts_req})

getenv            = False
environment       = "REPO=/home/gsmithline/perfsim CONDA_SH=/home/gsmithline/miniconda3/etc/profile.d/conda.sh ENV_NAME=opdyn"

output            = /home/gsmithline/perfsim/experiments/condor/logs/nodepack_{key}_$(group).out
error             = /home/gsmithline/perfsim/experiments/condor/logs/nodepack_{key}_$(group).err
log               = /home/gsmithline/perfsim/experiments/condor/logs/nodepack_{key}_$(group).log
...
queue group from experiments/condor/configs_pofd_{key}.txt
"""
...
def main():
    ...
    for r in rows_mix:
        _c = [c.strip() for c in r.split(",")]
        assert _c[1] == "sft" and _c[2] == "0", r     # ordinary SFT
        assert _c[5] == "replace", r                  # REF_REPLAY needs it
        assert _c[10] == "0.0", r                     # homophily gamma
        assert _c[11] == "1", r                       # W = 1
        assert _c[14] == "1", r                       # k = 1
    ...
    assert "REF_REPLAY_Q=$(refq)" in _mix_env, _mix_env
    assert f"REF_REPLAY_SEED={MIX_SEL_SEED} " in _mix_env
    assert f"REF_REPLAY_REF_RUN={MIX_REF_RUN} " in _mix_env
    assert "AB_SWEEPS=$(sweeps)" in _mix_env
    # REF_REPLAY refuses these; none may appear
    for _leak in ("REPLAY_FRAC", "PRISTINE_FRAC", "SFT_SAMPLE_N",
                  "SFT_EXCLUDE_CLAMPED"):
    ...
    files[p] = rows_mixs
    expected[p] = 1
    cube_subs[os.path.join(HERE, f"at_pofd_{MIX_SMOKE_KEY}.sub")] = mix_sub(smoke=True)
    # ---- forward-KL dose fill (LAM) ------------------------------------
    rows_lam = lam_rows()
    assert len(rows_lam) == 16, len(rows_lam)     # 4 new lambda x 4 (W,k)
    _lam_tags = [r.split(",")[0] for r in rows_lam]
    assert len(set(_lam_tags)) == 16, _lam_tags
    assert sum("_w1_k1_" in t for t in _lam_tags) == 4, _lam_tags
    assert sum("_w0p5_" in t for t in _lam_tags) == 12, _lam_tags
    for r in rows_lam:
        _c = [c.strip() for c in r.split(",")]
        assert _c[1] == "sft_kl", r
        assert float(_c[2]) in LAM_NEW, r          # only the NEW doses
        assert float(_c[2]) not in LAM_EXISTING, r # never re-run an old one
        assert _c[10] == "0.0", r                  # homophily gamma
        assert _c[15] == "forward", r              # forward KL only
        assert int(_c[16]) == LAM_SWEEPS, r
        assert _c[25] == "0", r                    # Qwen3 thinking OFF
        # the horizon must match the family this cell joins
        _w, _k, _r = float(_c[11]), float(_c[14]), int(_c[23])
        assert (_w, _k, _r) in LAM_FAMILIES, r
        assert f"_w{_num(_w)}_k{_num(_k)}_" in _c[0] and f"_r{_r}" in _c[0], r
    _lam_sub = lam_sub()
    _lam_env = next(l for l in _lam_sub.splitlines() if l.startswith("environment"))
    assert "AB_SWEEPS=$(sweeps)" in _lam_env and "KL_DIRECTION=$(kldir)" in _lam_env
    assert "REF_REPLAY" not in _lam_env
    _prior_lam = {r.split(",")[0] for rows in files.values() for r in rows}
    assert not (set(_lam_tags) & _prior_lam), set(_lam_tags) & _prior_lam
    p = os.path.join(HERE, f"configs_pofd_{LAM_KEY}.txt")
    files[p] = rows_lam
    expected[p] = 16
    cube_subs[os.path.join(HERE, f"at_pofd_{LAM_KEY}.sub")] = _lam_sub
    rows_lams = lam_smoke_rows()
    assert len(rows_lams) == 1
    _ls = [c.strip() for c in rows_lams[0].split(",")]
    assert _ls[0].startswith("pofdlamsmk_") and _ls[2] == "4"
    assert _ls[11] == "0.5" and _ls[14] == "0.2"
    assert _ls[23] == str(LAM_SMOKE_ROUNDS)
    assert _ls[0] not in set(_lam_tags)
    p = os.path.join(HERE, f"configs_pofd_{LAM_SMOKE_KEY}.txt")
    files[p] = rows_lams
    expected[p] = 1
    ...
    for _cond, _ncols in (("fixed", 26), ("evolving", 24)):
        ...
        assert "SAVE_RAW_GEN=1" in _env, ("parse_fail_frac lives only in "
                                          "raw_gen_log.json.gz", _cond)
    ...
    # ---- Section 3 open-gate cross-model equilibria (S3M) -------------
    assert tuple(S3M_MODELS) == tuple(FAM_MODELS), \
        "S3M must use the paper's exact six-model ordering"
    assert not S3M_REUSED, "the matched wave deliberately reuses no GPU cell"
    rows_s3m = s3m_rows()
    assert len(rows_s3m) == \
        len(S3M_MODELS) * len(S3M_SEEDS) == 18
    _s3m_tags = [r.split(",")[0].strip() for r in rows_s3m]
    assert len(set(_s3m_tags)) == 18
    _s3m_cells = set()
    for _r in rows_s3m:
        _c = [x.strip() for x in _r.split(",")]
        assert len(_c) == 29, (len(_c), _r)
        _t = _c[0]
        _model = next(m for m in S3M_MODELS if f"_{m}_" in _t)
        _seed = int(_c[3])
        _s3m_cells.add((_model, _seed))
        assert _t == s3m_tag(_model, _seed)
        assert _c[1] == "sft_kl" and float(_c[2]) == S3M_LAMBDA
        assert _seed in S3M_SEEDS
        assert float(_c[11]) == S3M_BETA
        assert float(_c[14]) == S3M_GAMMA
        assert _c[15] == "forward" and int(_c[16]) == S3M_SWEEPS
        assert int(_c[17]) == 0 and int(_c[19]) == 1
        assert int(_c[23]) == S3M_ROUNDS
        assert _c[24] == FAM_MODELS[_model]["base_model"]
        assert _c[25] == FAM_MODELS[_model]["chatthink"]
        assert "_eaopen_" in _t and "_esopen_" in _t
        assert f"_{S3_OP_TOKEN}_" in _t
    assert _s3m_cells == {(m, s) for m in S3M_MODELS for s in S3M_SEEDS}
    _prior_s3m = {r.split(",")[0] for rows in files.values() for r in rows}
    assert not (set(_s3m_tags) & _prior_s3m), set(_s3m_tags) & _prior_s3m
    _s3m_sub = s3m_sub()
    _s3m_env = next(l for l in _s3m_sub.splitlines()
                    if l.startswith("environment"))
    for _tok in ("AI_GATE_MODE=all_open", "PEER_GATE_MODE=all_open",
                 "AI_GATE_REFERENCE=anchor", "AB_SWEEPS=$(sweeps)",
                 "DEFFUANT_ALPHA=0.5",
                 "INNATE_LAMBDA=$(lam)", "KL_DIRECTION=$(kldir)",
                 "WITH_TWIN=1", "SAVE_RAW_GEN=1"):
        assert _tok in _s3m_env, (_tok, _s3m_env)
    assert "REF_REPLAY" not in _s3m_env and "INNATE_CLAMP" not in _s3m_env
    p = os.path.join(HERE, f"configs_pofd_{S3M_KEY}.txt")
    files[p] = rows_s3m
    expected[p] = 18
    cube_subs[os.path.join(HERE, f"at_pofd_{S3M_KEY}.sub")] = _s3m_sub

    rows_s3ms = s3m_smoke_rows()
    assert len(rows_s3ms) == 1
    _s3ms = [x.strip() for x in rows_s3ms[0].split(",")]
    assert _s3ms[0] == s3m_tag(S3M_SMOKE_MODEL, S3M_SMOKE_SEED,
                               rounds=S3M_SMOKE_ROUNDS, smoke=True)
    assert _s3ms[23] == str(S3M_SMOKE_ROUNDS)
    assert _s3ms[0] not in _prior_s3m and _s3ms[0] not in set(_s3m_tags)
    _s3ms_sub = s3m_sub(smoke=True)
    assert "check_section3_model_equilibria.py --smoke" in _s3ms_sub
    p = os.path.join(HERE, f"configs_pofd_{S3M_SMOKE_KEY}.txt")
    files[p] = rows_s3ms
    expected[p] = 1
    ...
    rows_ud = ud_rows()
    assert len(rows_ud) == 3, len(rows_ud)
    _ud_tags = [r.split(",")[0].strip() for r in rows_ud]
    assert len(set(_ud_tags)) == 3
    for _r in rows_ud:
        _c = [x.strip() for x in _r.split(",")]
        assert len(_c) == 30, (len(_c), _r)
        assert _c[0].startswith("pofdud_qwen3_8b_sft_u"), _r
        assert f"_{S3_OP_TOKEN}_" in _c[0] and _c[0].endswith("_r10"), _r
        assert _c[1] == "sft" and _c[2] == "0", _r     # plain SFT, no KL
        assert _c[11] == "1" and _c[14] == "1", _r     # W = 1, k = 1
        assert int(_c[16]) == 100, _r                  # S = 100
        _u = int(_c[0].split("_u")[1].split("_")[0])
        _acc = int(_c[17])
        assert (_u, _acc) in UD_ARMS, _r
        # realized steps = ceil(181/accum) must equal the tag's u
        assert -(-181 // _acc) == _u, (_u, _acc)
        assert _c[24] == str(UD_ROUNDS), _r
        assert _c[26] == "0", _r                       # Qwen3 thinking OFF
    _prior_ud = {r.split(",")[0] for rows in files.values() for r in rows}
    assert not (set(_ud_tags) & _prior_ud)
    assert UD_REUSED_U181 in _prior_ud, "the u181 reuse tag is not generated"
    _ud_sub = ud_sub()
    _ud_env = next(l for l in _ud_sub.splitlines()
                   if l.startswith("environment"))
    assert "SFT_GRAD_ACCUM=$(accum)" in _ud_env
    assert "SAVE_SFT_ORDER=1" in _ud_env and "SFT_EPOCHS=1" in _ud_env
    assert "SFT_MAX_STEPS" not in _ud_env, \
        "the step-CAP knob must never appear: it cuts data exposure"
    assert "AI_GATE_REFERENCE=anchor" in _ud_env
    p = os.path.join(HERE, f"configs_pofd_{UD_KEY}.txt")
    files[p] = rows_ud
    expected[p] = 3
    cube_subs[os.path.join(HERE, f"at_pofd_{UD_KEY}.sub")] = _ud_sub
    rows_uds = ud_smoke_rows()
    assert len(rows_uds) == 1
    _us = [x.strip() for x in rows_uds[0].split(",")]
    assert _us[0].startswith("pofdudsmk_") and _us[0].endswith("_r2")
    assert int(_us[17]) == 181 and _us[24] == "2"
    assert "check_update_dose.py --smoke" in ud_sub(smoke=True)
    p = os.path.join(HERE, f"configs_pofd_{UD_SMOKE_KEY}.txt")
    files[p] = rows_uds
    expected[p] = 1
    cube_subs[os.path.join(HERE, f"at_pofd_{UD_SMOKE_KEY}.sub")] = \
        ud_sub(smoke=True)
    # ---- UD seed replication (seeds 42/43 at U in {1, 5, 181}) ----------
    rows_udx = ud_ext_rows()
    assert len(rows_udx) == UD_N_EXT == 6, len(rows_udx)
    _udx_tags = [r.split(",")[0].strip() for r in rows_udx]
    assert len(set(_udx_tags)) == UD_N_EXT
    # never collide with the completed seed-0 cells or anything else
    assert not (set(_udx_tags) & set(_ud_tags))
    assert not (set(_udx_tags) & {r.split(",")[0]
                                  for rows in files.values() for r in rows})
    _seen_ux = set()
    for _r in rows_udx:
        _c = [x.strip() for x in _r.split(",")]
        ...
        assert _c[1] == "sft" and _c[2] == "0", _r     # plain SFT, no KL
        _sd = int(_c[3])
        assert _sd in UD_EXT_SEEDS, _r                 # seed col, not 0
        assert f"_s{_sd}_r10" in _c[0], _r             # tag carries the seed
        assert _c[5] == "replace" and _c[11] == "1" and _c[14] == "1", _r
        assert int(_c[16]) == 100, _r                  # S = 100
        _u = int(_c[0].split("_u")[1].split("_")[0])
        _acc = int(_c[17])
        assert (_u, _acc) in UD_EXT_ARMS, _r
        assert -(-181 // _acc) == _u, (_u, _acc)       # realized == tagged
        assert _c[24] == str(UD_ROUNDS), _r
        assert _c[26] == "0", _r                       # Qwen3 thinking OFF
        _seen_ux.add((_u, _sd))
    assert _seen_ux == {(u, s) for u, _ in UD_EXT_ARMS for s in UD_EXT_SEEDS}
    # the replication must reuse the SEED-0 TEMPLATE verbatim: same env
    # line, only the queue file and the gate flag change.
    _udx_sub = ud_sub(ext=True)
    _udx_env = next(l for l in _udx_sub.splitlines()
                    if l.startswith("environment"))
    assert _udx_env == _ud_env, "seed replication must reuse the UD env"
    assert "check_update_dose.py --seeds 42,43" in _udx_sub
    p = os.path.join(HERE, f"configs_pofd_{UD_SEEDS_KEY}.txt")
    files[p] = rows_udx
    expected[p] = UD_N_EXT
    ...
    # ---- F4A whole-node packing (A100 nodes accept whole-node jobs only) --
    _ng = f4a_node_groups()
    _all_prod = [r.split(",")[0].strip() for r in f4a_rows()]
    _packed = [e["tag"] for g, es_ in _ng.items() if g != "smoke0" for e in es_]
    assert _packed == _all_prod, "node groups must cover every production tag once, in grid order"
    assert all(1 <= len(v) <= F4A_NODE_CELLS_PER_JOB for v in _ng.values())
    assert len(_ng) == 9 and sorted(len(v) for g, v in _ng.items() if g != "smoke0") == [4] + [8] * 7
    assert [e["tag"] for e in _ng["smoke0"]] == \
        [r.split(",")[0].strip() for r in f4a_smoke_rows() + f4a_zsprior_rows()]
    for _g, _es in _ng.items():
        for _e in _es:
            assert os.path.exists(os.path.join(HERE, os.path.basename(_e["configs"]))) or \
                os.path.join(HERE, os.path.basename(_e["configs"])) in files, _e
    for _pair, _spec in F4A_NODE_PAIRS:
        for _smoke in (False, True):
            _rows = f4a_node_rows(_pair, _smoke)
            if not _rows:
                continue
            _sub = f4a_node_sub(_pair, _smoke)
            assert f"request_memory    = {_spec['memory_mb']}" in _sub
            assert f"request_disk      = {_spec['disk_kb']}" in _sub
            assert "request_gpus      = 8" in _sub and "request_cpus      = 128" in _sub
            assert F4A_GPU_NAME in _sub and "H100" not in _sub
            _k = (F4A_NODE_SMOKE_KEY if _smoke else F4A_NODE_KEY) + f"_{_pair}"
            p = os.path.join(HERE, f"configs_pofd_{_k}.txt")
            files[p] = _rows
            expected[p] = len(_rows)
            cube_subs[os.path.join(HERE, f"at_pofd_{_k}.sub")] = _sub
