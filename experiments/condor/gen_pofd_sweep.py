...
MEM_KS_NEW = [0.5]                # the only one not already run
MEM_ROUNDS = 60
MEM_SMOKE_ROUNDS = 3
MEM_SEED = 0
MEM_H100 = S3_H100
...
MEM_ARMS = [("sft", "sft", "0"), ("fwdlam1", "sft_kl", "1"),
            ("fwdlam8", "sft_kl", "8")]
MEM_KS_REUSED = (1.0, 0.2)


def mem_reused():
    """(arm, k) -> the archived Section 3 cell that serves that trained
    ...
    return {(arm, k): f"pofds3_qwen3_8b_{arm}_eaopen_w0p5_k{_num(k)}"
                      f"_esopen_{S3_OP_TOKEN}_s0_r100"
            for arm, _s, _l in MEM_ARMS for k in MEM_KS_REUSED}


def mem_tag(arm, k, rounds=MEM_ROUNDS, smoke=False):
    """pofdmem_qwen3_8b_fwdlam8_eaopen_w0p5_k0p5_esopen_anch2_s0_r60."""
    pre = "pofdmemsmk" if smoke else "pofdmem"
    return (f"{pre}_{MEM_MODEL}_{arm}_eaopen_w{_num(MEM_W)}_k{_num(k)}"
            f"_esopen_{S3_OP_TOKEN}_s{MEM_SEED}_r{rounds}")


def mem_row(arm, style, lam, k, rounds=MEM_ROUNDS, smoke=False):
    a = REACH_ARM_COLS["b1"]
    m = FAM_MODELS[MEM_MODEL]
    return ROW_S3.format(
        tag=mem_tag(arm, k, rounds, smoke), style=style, beta=lam,
        seed=MEM_SEED, es=f"{MEM_EPS_SOCIAL:g}", wplat=f"{MEM_W:g}",
        lam=f"{k:g}", kldir="forward", iclk=a["iclk"], snap=a["snap"],
        uselora=a["uselora"], fresh=a["fresh"], ansk=a["ansk"], gg=a["gg"],
        nrounds=rounds, basemodel=m["base_model"],
        chatthink=m["chatthink"], mem=m["mem"], disk=m["disk"],
        pplbatch=m["pplbatch"])


def mem_rows():
    return [mem_row(arm, style, lam, k)
            for k in MEM_KS_NEW for arm, style, lam in MEM_ARMS]


def mem_smoke_rows():
    """Strongest anchor at the NEW k: forward lambda = 8, k = 0.5."""
    return [mem_row("fwdlam8", "sft_kl", "8", 0.5,
...
# k = INNATE_LAMBDA rides the queue as $(lam). k = 1 and k = 0.2 are
# REUSED from the archived Section 3 cells (audited field by field), so
# only k = 0.5 is queued here.
# Submit: bash experiments/condor/submit_pofd_sweep.sh <BID> {key}
universe          = vanilla
executable        = /home/gsmithline/perfsim/experiments/condor/run_one_pokec_gated_idempotent.sh
arguments         = $(tag) $(style) $(beta) $(seed) $(deploy_every) $(regime) $(pscale) $(anchor) $(pop) $(eps) $(gamma) $(wplat) $(mode) $(canary)
...
def main():
    ...
    # ---- memory extension (MEM) ----------------------------------------
    rows_mem = mem_rows()
    assert len(rows_mem) == 3, len(rows_mem)         # 3 arms, k = 0.5 only
    _mem_tags = [r.split(",")[0] for r in rows_mem]
    assert len(set(_mem_tags)) == 3, _mem_tags
    assert all("_k0p5_" in t for t in _mem_tags), _mem_tags
    # k = 1 and k = 0.2 are REUSED and must never be queued
    assert not any(("_k1_" in t or "_k0p2_" in t) for t in _mem_tags), _mem_tags
    for r in rows_mem:
        _c = [c.strip() for c in r.split(",")]
        assert _c[10] == "0.0", r                    # homophily gamma
        assert _c[11] == "0.5", r                    # W = 0.5
        assert _c[14] == "0.5", r                    # k = 0.5
        assert _c[15] == "forward", r                # forward KL only
        assert "rev" not in _c[0], r
        assert _c[22] == str(MEM_ROUNDS), r
        assert _c[24] == "0", r                      # Qwen3 thinking OFF
    _mem_sub = mem_sub()
    _mem_env = next(l for l in _mem_sub.splitlines() if l.startswith("environment"))
    assert "INNATE_LAMBDA=$(lam)" in _mem_env
    assert "AB_SWEEPS" not in _mem_env               # one sweep: runner default
    _mem_q = next(l for l in _mem_sub.splitlines() if l.startswith("queue "))
    assert ", lam, kldir, iclk," in _mem_q, _mem_q
    ...
    assert _ms[0].startswith("pofdmemsmk_") and "_k0p5_" in _ms[0]
    assert _ms[1] == "sft_kl" and _ms[2] == "8" and _ms[14] == "0.5"
    assert _ms[22] == str(MEM_SMOKE_ROUNDS)
