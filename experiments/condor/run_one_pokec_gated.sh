...
# Args: RUN_TAG TRAINING_STYLE KL_BETA SEED DEPLOY_EVERY DATA_REGIME [PLATFORM_SUS_SCALE] [ANCHOR_MODE] [POP_MODEL] [EPS] [GAMMA_BIAS] [W_PLAT] [RUN_MODE] [CANARY_DELTA] [INNATE_LAMBDA] [FRESH_EACH_ROUND] [TRAIN_CAP] [PRISTINE_FRAC] [GEN_TEMPERATURE]
# Args 16-19 (fresh/cap/pristine/temp) override the env defaults; existing 15-arg subs use the env.
...
TRAIN_CAP="${17:-${TRAIN_CAP:-0}}"
N_PROBE="${N_PROBE:-64}"
TEL_EVAL_CAP="${TEL_EVAL_CAP:-64}"
GRAD_NORM_N="${GRAD_NORM_N:-8}"
FRESH_EACH_ROUND="${16:-${FRESH_EACH_ROUND:-0}}"
PRISTINE_FRAC="${18:-${PRISTINE_FRAC:-0.0}}"
