...
case "$WHAT" in
  ...
  # SECTION-3 PERSONAL-HISTORY ICL (2026-08-27): the Figure-3(a)
  # analogue with reference-regularized SFT replaced by FROZEN
  # personal-history ICL (ICL_K=0, ICL_DAYS=8, no LoRA/optimizer/KL).
  # Six checkpoints x seeds {0,42,43} = 18 jobs; every other field is
  # byte-matched to section3_model_equilibria. PARSE_MODE=strict
  # wave-wide -- no Mistral exemption. New pofds3i_ tags.
  # TWO DECODING ARMS, separate keys and explicit tag tokens:
  #   _greedy     DO_SAMPLE=0 -- the MAIN-PAPER arm
  #   _sample_t1  DO_SAMPLE=1, GEN_TEMPERATURE=1.0 -- ROBUSTNESS, one
  #               draw per agent per round; the run seed controls it
  # 18 jobs each = 36 production, plus one 3-round Mistral smoke per arm.
  # Flow (per arm): ..._smoke -> gate with check_section3_model_icl.py
  # --smoke --arm <arm> -> the 18-job key -> gate --arm <arm>.
  section3_model_icl_greedy|section3_model_icl_greedy_smoke) TARGETS="$WHAT" ;;
  # 100-ROUND HORIZON EXTENSION (2026-08-27) of the three models that
  # were unsettled at 30 rounds (Qwen3-8B, OLMo-2, OLMo-3) x 3 seeds = 9
  # jobs. Configuration UNCHANGED including AB_SWEEPS=100; only N_ROUNDS
  # moves. New _r100 tags -- the 30-round runs are never overwritten.
  # Gate with check_section3_model_icl.py --arm greedy --ext, which also
  # requires rounds 0..29 to be BIT-IDENTICAL to the 30-round runs.
  section3_model_icl_greedy_r100) TARGETS="$WHAT" ;;
  section3_model_icl_sample_t1|section3_model_icl_sample_t1_smoke) TARGETS="$WHAT" ;;
  ...
  # SECTION-4 PROBE (2026-08-26): 5-round seed-0 beta=0.75 channel probe
  # -- ea 0.7 x es {0,1} x arms {b0, d8} x conds {fixed, evolving} = 8
  # jobs (4 + 4). NO smoke: the 5-round wave IS the probe. PARSE_MODE=
  # strict + DEFFUANT_ALPHA=0.5 pinned in the env. New pofds4gp_ tags --
  # disjoint from every pofds4g_/pofds4gsmk_ tag, so nothing is shared
  # with (or overwritten by) the S4G/fig6 waves.
  # Flow: submit -> pull -> check_section4_gate.py --wave probe ->
  # analyze_s4g_probe.py.
  section4_gate_anch2_probe_fixed|section4_gate_anch2_probe_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_probe) TARGETS="section4_gate_anch2_probe_fixed section4_gate_anch2_probe_evo" ;;
  # SECTION-4 SCOUT (2026-08-27): the probe's parameters (beta=0.75,
  # gamma=0.2, alpha=0.5, ea=0.7) at the 30-ROUND production horizon
  # across five social gates es {0,.1,.2,.3,1} x {b0, d8} x {fixed,
  # evolving} = 20 jobs (10 + 10), seed 0. New pofds4gs_ tags -- disjoint
  # from the probe's pofds4gp_ (the wrapper would no-op a shared tag).
  # Decides whether beta=0.75 carries the full three-seed Figure-5 wave.
  # Flow: submit -> pull -> check_section4_gate.py --wave scout ->
  # analyze_s4g_probe.py --variant scout.
  section4_gate_anch2_scout_fixed|section4_gate_anch2_scout_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_scout) TARGETS="section4_gate_anch2_scout_fixed section4_gate_anch2_scout_evo" ;;
  # SECTION-4 SCOUT ON QWEN3-8B (2026-08-27): the scout grid on
  # Qwen/Qwen3-8B (CHAT_THINKING=0) after the Mistral scout failed the
  # strict raw-generation gate on 11/20 cells (deterministic d8
  # narrations + one whole malformed b0 round). 20 jobs (10 + 10), new
  # pofds4gq_ tags. Flow: submit -> pull -> check_section4_gate.py
  # --wave scout_qwen3 -> analyze_s4g_probe.py --variant scout_qwen3.
  section4_gate_anch2_scout_qwen3_fixed|section4_gate_anch2_scout_qwen3_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_scout_qwen3) TARGETS="section4_gate_anch2_scout_qwen3_fixed section4_gate_anch2_scout_qwen3_evo" ;;
  # THE EPS_AI SWEEP (2026-08-27): the same Qwen3 wave's SECOND arm --
  # es fixed at 0.3, ea in {0, .3, .5, 1} x {b0, d8} x {fixed, evolving}
  # = 16 jobs. The shared (ea .7, es .3) corner is NOT re-queued: it
  # already ran under the es-sweep key. ea=0 cells are WITNESSES (the
  # strict gate never opens, so op_raw must equal twin_raw bit-exactly).
  # Gate the union of both sweeps with --wave scout_qwen3.
  section4_gate_anch2_scout_qwen3_ea_fixed|section4_gate_anch2_scout_qwen3_ea_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_scout_qwen3_ea) TARGETS="section4_gate_anch2_scout_qwen3_ea_fixed section4_gate_anch2_scout_qwen3_ea_evo" ;;
  # SOCIAL-GATE EXTENSION (2026-08-27): es {.5, .7} at ea=.7, 30 rounds,
  # filling the four-fold gap between .3 and 1 in the Figure-5 curve.
  # 8 jobs; every other field identical to the completed es sweep, whose
  # 10 cells are NOT re-queued. Gate the union with --wave scout_qwen3.
  section4_gate_anch2_scout_qwen3_esx_fixed|section4_gate_anch2_scout_qwen3_esx_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_scout_qwen3_esx) TARGETS="section4_gate_anch2_scout_qwen3_esx_fixed section4_gate_anch2_scout_qwen3_esx_evo" ;;
  # GAMMA=1 ROUTING PILOT (2026-08-27): Qwen3-8B, beta(W_PLAT)=.5,
  # gamma(k)=1, alpha=.5, AI gate ALL_OPEN (tag _eaopen_, NOT ea1), one
  # Deffuant sweep, es {0,.1,.2,.3,1} x {b0,d8} x {fixed,evolving} = 20
  # jobs at 30 rounds, seed 0. New pofds4gg_ tags.
  # SMOKE FIRST: 4 jobs x 3 rounds at es=.2 (pofds4ggsmk_), gate with
  # check_section4_gate.py --wave pilot_g1 --smoke; submit the 20 only
  # if it passes. Then analyze_s4g_probe.py --variant pilot_g1 and
  # plot_s4g_pilot_g1.py for the MAE + routing-gap figure.
  section4_gate_anch2_pilot_g1_fixed|section4_gate_anch2_pilot_g1_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_pilot_g1_smoke_fixed|section4_gate_anch2_pilot_g1_smoke_evo) TARGETS="$WHAT" ;;
  section4_gate_anch2_pilot_g1) TARGETS="section4_gate_anch2_pilot_g1_fixed section4_gate_anch2_pilot_g1_evo" ;;
  section4_gate_anch2_pilot_g1_smoke) TARGETS="section4_gate_anch2_pilot_g1_smoke_fixed section4_gate_anch2_pilot_g1_smoke_evo" ;;
  ...
  # LoRA-rank robustness for Figure 3 (2026-08-28): r=16 at lambda
  # {0,2}, beta=gamma=1, everything else identical to the figure.
  # Flow: fig3_rank16_smoke (1 x 3 rounds, r=16 lambda=2) -> gate
  # with check_fig3_full_loop.py --wave rank16 --smoke -> fig3_rank16.
  # The lambda=inf endpoint is rank-independent and is NEVER queued.
  fig3_rank16|fig3_rank16_smoke) TARGETS="$WHAT" ;;
  # corrected continual-adaptation control (2026-08-29): fresh vs
  # carried adapter weights under anch2, lambda {0,2} x 3 seeds, on
  # the NUMERIC-gate fec surface where the correction can bite.
  # Flow: continual_anch2_smoke (1 x 3 rounds, continual lambda=2)
  # -> gate with check_continual_anch2.py --smoke -> continual_anch2.
  continual_anch2|continual_anch2_smoke) TARGETS="$WHAT" ;;
  ...
  sft_update_dose_loop|sft_update_dose_loop_smoke|sft_update_dose_loop_seeds) TARGETS="$WHAT" ;;
  ...
  # WHOLE-NODE packing on g181-g184 (A100 nodes accept whole-node jobs only;
  # H100 pool absent 2026-08-26): 8 cells per node job via node_pack.py.
  fig4_anchor_tradeoff_node_a|fig4_anchor_tradeoff_node_b|fig4_anchor_tradeoff_node_smoke_a) TARGETS="$WHAT" ;;
  fig4_anchor_tradeoff_node)       TARGETS="fig4_anchor_tradeoff_node_a fig4_anchor_tradeoff_node_b" ;;
  fig4_anchor_tradeoff_node_smoke) TARGETS="fig4_anchor_tradeoff_node_smoke_a" ;;
  # sw1 sibling wave (one Deffuant sweep per round): separate tags/subs/manifest
  fig4_anchor_tradeoff_sw1|fig4_anchor_tradeoff_sw1_ext|fig4_anchor_tradeoff_sw1_node_a|fig4_anchor_tradeoff_sw1_node_b|fig4_anchor_tradeoff_sw1_node_smoke_a) TARGETS="$WHAT" ;;
  fig4_anchor_tradeoff_sw1_smoke)      TARGETS="fig4_anchor_tradeoff_sw1_smoke fig4_anchor_tradeoff_zsprior" ;;
  fig4_anchor_tradeoff_sw1_node)       TARGETS="fig4_anchor_tradeoff_sw1_node_a fig4_anchor_tradeoff_sw1_node_b" ;;
  fig4_anchor_tradeoff_sw1_node_smoke) TARGETS="fig4_anchor_tradeoff_sw1_node_smoke_a" ;;
  # second-half cells as single-GPU jobs (capacity move); remove the idle node groups prod4-7 FIRST
  fig4_anchor_tradeoff_rest|fig4_anchor_tradeoff_sw1_rest|fig4_anchor_tradeoff_first|fig4_anchor_tradeoff_sw1_first) TARGETS="$WHAT" ;;
